...
function renderToys() {
    const toys = getToys()
    const strHTMLs = toys.map(toy => {
        const className = (toy.price < 20) ? 'success' : 'danger'
        const checked = (toy.isSelected) ? 'checked' : ''
        return `<tr class="row">
                    <td> ${toy.name}</td>
